            ...
            <div>
                @foreach ($unreadNotification as $item)
                    <div class="px-2 py-2 my-1 bg-slate-200">
                        <a href="{{route('read',['notificationId' => $item->id,'postId' => $item->data['post_id']])}}" class="text-darkgb">
                           
                            <p>
                                <span class="font-bold hover:text-blue">{{$item->data['commentator']}}</span>
                                <span class="text-darkgb">{{$item->data['type']}}</span>
                            </p>
                        </a>
                    </div>
                @endforeach
